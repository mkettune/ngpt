...
public:
	...
	virtual void setBitmapMulti(const Bitmap *bitmap, Float multiplier, int buf){
		Log(EWarn, "setBitmapMulti() is only implemented for MultiFilm");
	}
	virtual void addBitmapMulti(const Bitmap *bitmap, Float multiplier, int buf){
		Log(EWarn, "addBitmapMulti() is only implemented for MultiFilm");
	}
	virtual bool developMulti(const Point2i &offset, const Vector2i &size, const Point2i &targetOffset, Bitmap *target, int buf) const{
	...
	virtual bool setBuffers(std::vector<std::string> &names, std::vector<bool> &outWrite){
		Log(EWarn, "setBuffers() is only implemented for MultiFilm.");
		return false;
	}

	/// rename buffer function for MultiFilm. Usage: Integrator passes list of output buffernames to the Film.
	virtual bool renameBuffers(std::vector<std::string> &names, std::vector<bool> &outWrite){
		Log(EWarn, "renameBuffers() is only implemented for MultiFilm.");
		return false;
	}
