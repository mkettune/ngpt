...
public:
	...
	/**
	 * \brief Reset the position of the random number sequence for deterministic replay.
	 *
	 * This function is used to provide each pixel a deterministic sequence of
	 * repeatable random numbers such that we can repeat the same sequence of paths
	 * for the temporal base and offset images.
	 */
	virtual void resetRNG(const Point2i &pos, int seed, int frame, int sample);
