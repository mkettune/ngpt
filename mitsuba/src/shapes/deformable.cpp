...
public:
	...
	void adjustTime(Intersection &its, Float time) const {
		...
		cache.frameIndex = m_kdtree->findFrame(its.time); //marco: FIXED! (cast its.time to integer)
		...
		cache.frameIndex = m_kdtree->findFrame(time); //marco FIXED! (cast time to integer)
		cache.alpha = std::max((Float) 0.0f, std::min((Float) 1.0f,
