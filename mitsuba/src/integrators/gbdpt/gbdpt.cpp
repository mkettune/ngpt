 ...
 * implementation that hasn't been tested with all of Mitsuba's features.
 ...
 * Namely G-BDPT generates 7 images: the primal image, 4 gradient images, the unbiased reconstructed image using the L2 norm and the biased reconstruction using the L1 norm.
...
public:
    GBDPTIntegrator(const Properties &props) : Integrator(props) {
		Log(EError, "Temporal gradient-domain bidirectional path tracing not implemented!");
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
		...
		std::vector<bool> outWrite ={ 1, 1, 1, 1, 1, 1 ,1 };
