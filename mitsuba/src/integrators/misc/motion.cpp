...
public:
	...
	MotionIntegrator(const Properties &props) : SamplingIntegrator(props) {
		...
		m_config = boost::to_lower_copy(props.getString("config", "rd"));
		...
		m_glossyThreshold = props.getFloat("glossyThreshold", 0.1);
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
		...
		//sample_config = getConfiguration(rRec, r, max_nbounces); //marco: turned off since this plugin cannot handle many materials correctly... :-(
		sample_config = "d";

		if (sample_config.find_first_of("d") != sample_config.length() - 1 || sample_config.length()>max_nbounces+1)
			Log(EError, "configuration invalid: %s", sample_config.c_str());
		
		if (sample_config.length() == 1) {
			...
			if (!tracePath(rRec, ray, source, sample_config))
			...
			while (true) {
				...
				if(!adjustTime(rRec, apertureSample, source, target, timeStepSize))
					return Spectrum(0.0f);
				...
				if (timeIteration == 1) {
					...
					if (m_derivativesOnly) {
						p1 = extrapolateTimePoint(source, target, sample_config);
						break;
					} else {
				...
				if (!timeStep(rRec, source, target, temp, temp2, sample_config)) {
		...
		//marco: method seems to return nonsense when offset is out of image -> set motion vector to zero then!
		if (dRec1.pdf==0.f)
			return result;
		...
		//marco: we are interested in the distance of the primary point to the camera, the depth difference doesn't matter
		//i.e. we get a depth map for free in the B channel
		dDelta = dRec0.dist;
	...
	std::string getConfiguration(RadianceQueryRecord &rRec_start, Ray ray_origin, int maxDepth) const{
		Ray ray(ray_origin);
		std::string config = "";
		int depth=0;
		RadianceQueryRecord rRec(rRec_start);
		...
		while (depth < maxDepth){
			...
			if (!its.isValid() || its.isEmitter() && !(its.shape->getBSDF()->getType() & BSDF::EDiffuseReflection))
				break;

			const BSDF *bsdf = its.shape->getBSDF();

			//check component with lowest roughness
			Float minRoughness = std::numeric_limits<Float>::infinity();
			for (int i=0; i < bsdf->getComponentCount(); i++){
				Float currentRoughness = bsdf->getRoughness(its, i);
				minRoughness = currentRoughness < minRoughness ? currentRoughness : minRoughness;
			}
			if (minRoughness > m_glossyThreshold){
				break;
			}
			...
			//dielectric material -> sample transmission component.
			...
			if (bRec.typeMask == BSDF::EAll){
				//deprecated since we do not use phong anymore...
				if (bsdf->getDiffuseReflectance(its).getLuminance() > bsdf->getSpecularReflectance(its).getLuminance()){
					break;
				}
				else if (bsdf->getType() & BSDF::EDeltaReflection) {
					current_type = "r";
			...
			//sample surface
			Spectrum bsdfWeight = bsdf->sample(bRec, Point2(0.0f)); //marco: fix for phong.

			if (bsdfWeight.isZero() || bRec.wo.isZero() || its.isEmitter() || !its.isValid())
				break;

			config += current_type;

			//Log(EWarn, "%s", bRec.wo.toString().c_str());
		...
		//last element must always be diffuse
		config += "d";

		//if (config.find_first_of("t")!=std::string::npos)
		//	Log(EWarn, "config: %s", config.c_str());

		return config;
	}

	bool timeStep(RadianceQueryRecord &rRec, std::vector<Intersection> &source, const std::vector<Intersection> &target, std::vector<Intersection> &temp, std::vector<Intersection> &temp2, const std::string &sample_config) const {
		Ray ray = extrapolateTimeRay(source, target, sample_config);

		if (!tracePath(rRec, ray, temp, sample_config))
			return false;
		...
		while (error > 1e-5f) {
			...
			Ray candidateRay = extrapolateSpaceRay(temp, target, spaceStepSize, sample_config);

			Float candidateError = 0;
			if (!tracePath(rRec, candidateRay, temp2, sample_config))
	...
	bool tracePath(RadianceQueryRecord &rRec, Ray ray, std::vector<Intersection> &intersections, const std::string &sample_config) const {
		int depth = 0;
		...
		while (depth < (int) sample_config.size()) {
			rRec.scene->rayIntersect(ray, its);

			char interactionType = sample_config[depth++];
	...
	bool adjustTime(const RadianceQueryRecord &rRec, const Point2 &apertureSample, const std::vector<Intersection> &source, std::vector<Intersection> &target, Float timeStepSize) const {
		target = source;
		...
		sensor->sampleDirect(dRec0, apertureSample).isZero();
		sensor->sampleDirect(dRec1, apertureSample).isZero();

		if (sensor->sampleDirect(dRec0, apertureSample).isZero() || sensor->sampleDirect(dRec1, apertureSample).isZero())
			return false;
	...
	void assembleMatrix(const std::vector<Intersection> &source, const std::vector<Intersection> &target, EMatrix &M, const std::string sample_config) const {
	...
	Point extrapolateTimePoint(const std::vector<Intersection> &source, const std::vector<Intersection> &target, const std::string &sample_config) const {
		EMatrix M;
		assembleMatrix(source, target, M, sample_config);
	...
	Ray extrapolateTimeRay(const std::vector<Intersection> &source, const std::vector<Intersection> &target, const std::string &sample_config) const {
		EMatrix M;
		assembleMatrix(source, target, M, sample_config);
	...
	Ray extrapolateSpaceRay(const std::vector<Intersection> &source, const std::vector<Intersection> &target, Float stepSize, const std::string &sample_config) const {
		EMatrix M;
		assembleMatrix(source, target, M, sample_config);
