...
#ifdef _WIN32 // Solver only enabled in Windows.
#include "Solver.hpp"
#endif
...
/// Evaluates variances of Spectra on-line.
template<typename T>
class Variance {
public:
	...
	T getSampleVarianceOfMean() const {
		if(n < 2) {
			//Log(EWarn, "Sample variance with too few samples.");
			return T((Float)(-1));
		}
		return M2 / (Float)(n*(n - 1));
	}
...
/// sets a bitmap from a given array data
...
/// constructs a path to the base passes sampling or motion map, 
/// assumes file structure of Markus' scripts (e.g. the sampling map of the base pass is assumed to be in ..//frame(XX-1)_seedYY//image-spp.exr ) 
fs::path getMapPath(const Scene* scene, const int frame, const int seed, const std::string suffix){
	fs::path outname = scene->getDestinationFile();
	fs::path basename = outname.leaf();
	outname = outname.branch_path();
	std::string dirname = outname.leaf().string();
	outname = outname.branch_path();
	fs::path outname_old = outname;

	int frame_nr = std::stoi(dirname.substr(5, 7)); //first number in name frameXXX_seedYY =>
	int seed_nr = std::stoi(dirname.substr(13, dirname.length())); //2nd number in name frameXXX_seedYY => seed number

	SLog(EWarn,"frame number: %d, seed number: %d", frame_nr, seed_nr);

	std::string fidx = (frame_nr - 1 < 10 ? "00" : (frame_nr - 1 < 100 ? "0" : "")) + std::to_string(frame_nr - 1);
	outname /= "frame" + fidx + "_seed" + std::to_string(seed_nr);
	outname /= basename;
	outname +=  suffix + ".exr";

	//use frame-1 if we are currently in one of the time-offset passes
	if (!fs::exists(outname)){
		fidx = (frame_nr < 10 ? "00" : (frame_nr < 100 ? "0" : "")) + std::to_string(frame_nr);
		outname_old /= "frame" + fidx + "_seed" + std::to_string(seed_nr);
		outname_old /= basename;
		outname_old += suffix + ".exr";
		outname = outname_old;
	}
...
struct RayState {
	RayState()
		: radiance(0.0f),
		radianceDiffuse(0.0f),
		gradient(0.0f),
		eta(1.0f),
		pdf(1.0f),
		throughput(Spectrum(0.0f)),
		throughputDiffuse(Spectrum(0.0f)),
		throughput_NoJ(Spectrum(0.0f)),
		alive(true),
		roughFound(false),
		...
		jacobianSum(0.0f),
		jacobianMax(0.0f),
		jacobianMin(10000000.0f),
		jacobianInverseSum(0.0f),
		jacobianSquareSum(0.0f),
		jacobianInverseSquareSum(0.0f),
		jacobianCount(0)
	{}
	...
	inline void addDiffuseRadiance(const Spectrum& contribution, Float weight) {
		Spectrum color = contribution * weight;
		radianceDiffuse += color;
	}
	...
	Spectrum throughputDiffuse;      ///< Diffuse throughput of the path.
	Spectrum throughput_NoJ;         ///< Current throughput of the path.
	...
	Spectrum radianceDiffuse;        ///< Diffuse component of radiance.
	...
	bool roughFound;                 ///< Indicates whether the camera path contains a rough vertex (diffuse radiance can be accumulated)
	...
	double jacobianMax;
	double jacobianMin;
	double jacobianInverseSum;
	double jacobianSquareSum;
	double jacobianInverseSquareSum;
	size_t jacobianCount;
};
...
Float getLowestRoughness(const BSDF* bsdf, Intersection& its, const GradientPathTracerConfig& config, unsigned int bsdfType) {
...
VertexType getVertexType(const BSDF* bsdf, Intersection& its, const GradientPathTracerConfig& config, unsigned int bsdfType) {
	...
	Float lowest_roughness = getLowestRoughness(bsdf, its, config, bsdfType);
...
/// Returns the vertex type of a vertex by its roughness value. For feature output.
VertexType getVertexTypeForFeatures(RayState &ray, const GradientPathTracerConfig& config) {
	const BSDF* bsdf = ray.rRec.its.getBSDF(ray.ray);
	Float lowest_roughness = getLowestRoughness(bsdf, ray.rRec.its, config, BSDF::ESmooth);
	
	if (lowest_roughness <= config.m_shiftThreshold) { // TODO: Use some other value?
		return VERTEX_TYPE_GLOSSY;
	}
	else {
		return VERTEX_TYPE_DIFFUSE;
...
/// Updates the features from the intersection.
void updateFeatures(Spectrum* features, const Transform& sensorViewTransform, const RayState& ray) {
	const Intersection& its = ray.rRec.its;
	
	// TODO: constants for feature indices.

	// Albedo.  TODO: Multiply by specular albedos encountered so far?
	features[0] = Spectrum(its.getBSDF()->getDiffuseReflectance(its));

	// Transform the hit position into sensor's local space.
	Point pointInLocalSpace = sensorViewTransform(its.p);
	Spectrum pos = Spectrum(0.0f);
	pos[0] = pointInLocalSpace.x; pos[1] = pointInLocalSpace.y; pos[2] = pointInLocalSpace.z;
	features[2] = pos;

	// Transform the normal into sensor's local space.
	Normal normalInLocalSpace = sensorViewTransform(its.shFrame.n);

	// Flip all normals so that they point towards the camera.
	Vector3 itsDirectionInLocalSpace = pointInLocalSpace - Point3((Float)0, (Float)0, (Float)0);
	itsDirectionInLocalSpace /= (Float)1e-10 + itsDirectionInLocalSpace.length();

	if(dot(itsDirectionInLocalSpace, normalInLocalSpace) >= (Float)0) {
		normalInLocalSpace = -normalInLocalSpace;
	}

	// Store normal.
	Float normal_vec[3] = {normalInLocalSpace.x, normalInLocalSpace.y, normalInLocalSpace.z};
	features[1] = Spectrum(normal_vec);

	// Depth.
	features[3] += Spectrum(its.t);

	// Diffuse.
	features[5] = ray.radianceDiffuse;
	// Specular.
	features[6] = ray.radiance - ray.radianceDiffuse;
}

/// Updates the features for an environment hit straight after the sensor.
void updateFeaturesEnvironment(Spectrum* features, const Transform& sensorViewTransform, const RayState& ray) {
	const Intersection& its = ray.rRec.its;

	// Albedo.
	features[0] = Spectrum(0.0f);

	// Position.
	features[2] = Spectrum(0.0f);

	// Normal.
	Normal normalInLocalSpace = sensorViewTransform(-ray.ray.d);
	Float normal_vec[3] = {normalInLocalSpace.x, normalInLocalSpace.y, normalInLocalSpace.z};
	features[1] = Spectrum(normal_vec);

	// Depth.
	//features[3] += Spectrum(0.0f);
...
/// Updates the visibility feature.
void updateFeatureVisibility(Spectrum* features, int shadowRaySuccessCount, int shadowRayCount) {
	// Visibility buffer.
	Float visibility = (shadowRayCount > 0) ? (Float)(shadowRaySuccessCount) / (Float)shadowRayCount : (Float)1;
	features[4] = Spectrum(visibility);
}

/// Updates the diffuse and specular features.
void updateFeaturesDiffuseSpecular(Spectrum* features, const RayState& ray) {
	// Diffuse.
	features[5] = ray.radianceDiffuse;
	// Specular.
	features[6] = ray.radiance - ray.radianceDiffuse;
...
public:
	...
	void evaluatePoint(RadianceQueryRecord& rRec, const Point2& samplePosition, const Point2& apertureSample, Float timeSample, Float differentialScaleFactor,
		Spectrum& out_very_direct, Spectrum& out_throughput, Spectrum *out_gradients, Spectrum *out_neighborThroughputs, Float *alphas, Spectrum *out_features, bool *out_featuresWritten, Spectrum *out_debug, bool compute_shifts)
	{
		// Initialize the base path.
		RayState mainRay;
		mainRay.throughput = m_sensor->sampleRayDifferential(mainRay.ray, samplePosition, apertureSample, timeSample);
		mainRay.throughputDiffuse = mainRay.throughput;
		...
		for(int i = 0; i < 4; ++i) {
			...
			shiftedRays[i].throughput_NoJ = shiftedRays[i].throughput;
		...
		const Transform& sensorWorldTransform = m_sensor->getWorldTransform()->eval(timeSample);
		Spectrum very_direct = Spectrum(0.0f);
		evaluate(mainRay, shiftedRays, 4, very_direct, alphas, sensorWorldTransform, out_features, out_featuresWritten, out_debug, compute_shifts);
	...
	inline BSDFSampleResult sampleBSDF(RayState& rayState) {
		...
		SAssert(result.pdf <= (Float)0 || std::abs(result.bRec.wo.length() - 1.0) < 0.00001);
		return result;
	}
	...
	void evaluate(RayState& main, RayState* shiftedRays, int secondaryCount, Spectrum& out_veryDirect, Float *alphas, const Transform& sensorWorldTransform, Spectrum *out_features, bool* out_featuresWritten, Spectrum *out_debug, bool compute_shifts) {
		...
		Spectrum mainVeryDirect((Float)0); // diffuse buffer ignores very direct

		// For the visibility map for features.
		int feature_shadowRayCount = 0;
		int feature_shadowRaySuccessCount = 0;
		...
		if (!main.rRec.its.isValid()) {
			...
			if (main.rRec.type & RadianceQueryRecord::EEmittedRadiance) {
				mainVeryDirect += main.throughput * scene->evalEnvironment(main.ray);
			}
			...
			if (main.rRec.its.isEmitter() && (main.rRec.type & RadianceQueryRecord::EEmittedRadiance)) {
				mainVeryDirect += main.throughput * main.rRec.its.Le(-main.ray.d);
			}
			...
			if (main.rRec.its.hasSubsurface() && (main.rRec.type & RadianceQueryRecord::ESubsurfaceRadiance)) {
				mainVeryDirect += main.throughput * main.rRec.its.LoSub(scene, main.rRec.sampler, -main.ray.d, 0);
			}
		}
		main.addRadiance(mainVeryDirect, (Float)1);


		// Evaluate throughputs for offset paths.
		for(int i = 0; i < secondaryCount; ++i) {
			RayState& shifted = shiftedRays[i];

			Spectrum shiftedVeryDirect((Float)0);

			if (!shifted.rRec.its.isValid()) {
				// Add potential very direct light from the environment as gradients are not used for that.
				if (shifted.rRec.type & RadianceQueryRecord::EEmittedRadiance) {
					shiftedVeryDirect += shifted.throughput * scene->evalEnvironment(shifted.ray);
				}
			} else {
				// Add very direct light from non-environment.

				// Include emitted radiance if requested.
				if (shifted.rRec.its.isEmitter() && (shifted.rRec.type & RadianceQueryRecord::EEmittedRadiance)) {
					shiftedVeryDirect += shifted.throughput * shifted.rRec.its.Le(-shifted.ray.d);
				}

				// Include radiance from a subsurface scattering model if requested. Note: Not tested!
				if (shifted.rRec.its.hasSubsurface() && (shifted.rRec.type & RadianceQueryRecord::ESubsurfaceRadiance)) {
					shiftedVeryDirect += shifted.throughput * shifted.rRec.its.LoSub(scene, shifted.rRec.sampler, -shifted.ray.d, 0);
				}
			}

			shifted.addGradient(shiftedVeryDirect - mainVeryDirect, (Float)1);

#ifndef CENTRAL_RADIANCE
			shifted.addRadiance(shiftedVeryDirect);
#endif
		}

		// Update features.
		Transform sensorViewTransform = sensorWorldTransform.inverse();
		bool stopFeatureUpdates = false;
		
		if(out_features) {
			SAssert(out_featuresWritten);
			*out_featuresWritten = false; // TODO: Remove, as always written.

			if (!main.rRec.its.isValid()) {
				// Environment hit.
				updateFeaturesEnvironment(out_features, sensorViewTransform, main);
				*out_featuresWritten = true;
				return;
			}
			
			if (!stopFeatureUpdates) {
				updateFeatures(out_features, sensorViewTransform, main);
				if(getVertexTypeForFeatures(main, *m_config) == VERTEX_TYPE_DIFFUSE) {
					stopFeatureUpdates = true;
				}
				*out_featuresWritten = true;
			}
		}
		...
		while(main.rRec.depth < m_config->m_maxDepth || m_config->m_maxDepth < 0) {
			...
			// Sample incoming radiance from lights (next event estimation).
			{
				...
				if ( main.rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance && mainBSDF->getType() & BSDF::ESmooth && main.rRec.depth + 1 >= m_config->m_minDepth) {
					...
					// Add to visibility feature buffer.
					++feature_shadowRayCount;
					if(mainEmitterVisible) {
						++feature_shadowRaySuccessCount;
					}
					...
					// Get diffuse contribution of NEE
					if (main.roughFound) {
						// Already past first rough vertex => accumulate all radiance
						main.addDiffuseRadiance(main.throughputDiffuse * (mainBSDFValue * mainEmitterRadiance), mainWeightNumerator / (D_EPSILON + mainWeightDenominator));
					}
					else if (mainBSDF->hasComponent(BSDF::EDiffuse)) {
						// At first non-specular vertex => get purely diffuse contribution
						BSDFSamplingRecord diffBRec = mainBRec;
						diffBRec.typeMask = BSDF::EDiffuse;

						const Spectrum diffBsdfVal = mainBSDF->eval(diffBRec);

						// TODO: Implement strict normals check here and above. Make sure that the offset paths below are consistent.
						//if (!diffBsdfVal.isZero() && (!m_config->m_strictNormals
						//	|| dot(main.rRec.its.geoFrame.n, dRec.d) * Frame::cosTheta(diffBRec.wo) > 0))
						{
							main.addDiffuseRadiance(main.throughputDiffuse * (diffBsdfVal * mainEmitterRadiance), mainWeightNumerator / (D_EPSILON + mainWeightDenominator));
						}
					}

					// Strict normals check to produce the same results as bidirectional methods when normal mapping is used.
					if(compute_shifts && (!m_config->m_strictNormals || dot(main.rRec.its.geoFrame.n, dRec.d) * Frame::cosTheta(mainBRec.wo) > 0)) {
						// The base path is good. Add radiance differences to offset paths.
						for(int i = 0; i < secondaryCount; ++i) {

							bool shiftedEmitterVisible = true;
							//MARCO: pre-compute alphas
							Float baseAlpha = alphas[i];
							Float baseAlphaSqr = alphas[i] * alphas[i];
							Float offsetAlphaSqr = (1.f-alphas[i]) * (1.f-alphas[i]);
							...
							Spectrum shiftedContribution_NoJ(Float(0)); //shifted contribution without Jacobian determinant
							Float weight = Float(0);

							Float jacobianDet = shifted.jacobianDet;
							...
							if(shiftSuccessful) {
								...
								if(shifted.connection_status == RAY_CONNECTED) {
									...
									//MARCO: changed to include probabilities of sampling a direction ("alphas")
									weight = mainWeightNumerator*baseAlpha / (D_EPSILON + shiftedWeightDenominator*offsetAlphaSqr + mainWeightDenominator*baseAlphaSqr);
									...
									shiftedContribution_NoJ = shifted.throughput_NoJ * (shiftedBsdfValue * shiftedEmitterRadiance); //Marco: offset TP without jacobian
									...
									if(mainAtPointLight || (mainVertexType == VERTEX_TYPE_DIFFUSE && shiftedVertexType == VERTEX_TYPE_DIFFUSE)) {
										...
										shiftedEmitterVisible = emitterTuple.second;
							...
							if(!shiftSuccessful) {
								...
								//MARCO: changed to include probabilities of sampling a direction ("alphas"). Is this valid here?
								weight = mainWeightNumerator*baseAlpha / (D_EPSILON + mainWeightDenominator*baseAlphaSqr);

								//weight = mainWeightNumerator / (D_EPSILON + mainWeightDenominator);

								mainContribution = main.throughput * (mainBSDFValue * mainEmitterRadiance);
								shiftedContribution = Spectrum((Float)0);
								shiftedContribution_NoJ = Spectrum(Float(0.f));//Marco: offset TP without jacobian
			...
			const BSDF* previousBSDF = previousMainIts.getBSDF(main.ray);

			// Check if this is the first non-specular vertex
			bool isPureDelta = !previousBSDF->hasComponent(BSDF::ESmooth);
			if (!main.roughFound && !isPureDelta) {
				BSDFSamplingRecord diffBRec = mainBsdfResult.bRec;
				diffBRec.typeMask = BSDF::EDiffuse;
				bool sampledIsDelta = (mainBsdfResult.bRec.sampledType & BSDF::ESmooth) == 0;

				// Weight must contain continuation ray pdf, not ideal diffuse pdf?
				Spectrum diffWeight = previousBSDF->eval(diffBRec);
				
				main.throughputDiffuse *= (sampledIsDelta) ? Spectrum(0.0f) : (diffWeight);
				main.roughFound = true;
			}
			else {
				main.throughputDiffuse *= mainBsdfResult.weight * mainBsdfResult.pdf; // bsdf * pdf saved?
			}
			...
			if (scene->rayIntersect(main.ray, main.rRec.its)) {
				...
				// Update features at first non-specular interaction.
				if (out_features && !stopFeatureUpdates) {
					updateFeatures(out_features, sensorViewTransform, main);
					updateFeatureVisibility(out_features, feature_shadowRaySuccessCount, feature_shadowRayCount);
			...
			if (main.roughFound) {
				main.addDiffuseRadiance(main.throughputDiffuse * mainEmitterRadiance, mainWeightNumerator / (D_EPSILON + mainWeightDenominator));
			}
			...
			for (int i = 0; i < secondaryCount; ++i) {
				if (compute_shifts){
					...
					Spectrum mainContribution(Float(0));
					Spectrum shiftedContribution(Float(0));
					Spectrum shiftedContribution_NoJ(Float(0));
					Float weight(0);
					...
					if (shifted.alive) {
						...
						if (shifted.connection_status == RAY_CONNECTED) {
							...
							shifted.throughput_NoJ *= shiftedBsdfValue; //Marco: offset TP without jacobian
							...
							shiftedContribution_NoJ = shifted.throughput_NoJ * shiftedEmitterRadiance; //Marco: offset TP without jacobian
							...
							if (mainVertexType == VERTEX_TYPE_DIFFUSE && mainNextVertexType == VERTEX_TYPE_DIFFUSE && shiftedVertexType == VERTEX_TYPE_DIFFUSE) {
								...
								if (!lastSegment || mainHitEmitter || main.rRec.its.hasSubsurface()) {
									...
									shifted.jacobianDet *= shiftResult.jacobian; //Marco: store jacobian
								...
								if (shiftResult.success) {
									...
									shifted.throughput_NoJ *= 1.f; //Marco: offset TP without jacobian
									shifted.pdf *= shiftResult.jacobian;
									shifted.jacobianDet *= shiftResult.jacobian; //Marco: jacobian itself
								...
								// Update throughput and pdf.
								{
									...
									shifted.throughput_NoJ *= shiftedBSDF->eval(bRec, measure); //Marco: offset TP without jacobian
							...
							half_vector_shift_failed:
								if (shifted.alive) {
									...
									//MARCO: changed to include probabilities of sampling a direction ("alphas")
									weight = main.pdf * baseAlpha / (D_EPSILON + shifted.pdf * shifted.pdf * offsetAlphaSqr + main.pdf * main.pdf * baseAlphaSqr);
									mainContribution = main.throughput * mainEmitterRadiance;
									shiftedContribution = shifted.throughput * shiftedEmitterRadiance; // Note: Jacobian baked into .throughput.
									shiftedContribution_NoJ = shifted.throughput_NoJ * shiftedEmitterRadiance;  //Marco: offset TP without jacobian
								}
									...
									//MARCO: changed to include probabilities of sampling a direction ("alphas")
									weight = Float(1) / (D_EPSILON + main.pdf*baseAlpha);
									mainContribution = main.throughput * mainEmitterRadiance;
									shiftedContribution = Spectrum(Float(0));
									shiftedContribution_NoJ = Spectrum(Float(0.f));//Marco: offset TP without jacobian
									...
									//	failedShifts[i].addGradient(-mainContribution, weight); //Marco: store fails
		...
		// Update visibility feature.
		if(out_features && *out_featuresWritten) {
			updateFeatureVisibility(out_features, feature_shadowRaySuccessCount, feature_shadowRayCount);
			updateFeaturesDiffuseSpecular(out_features, main);
		}
	...
	: MonteCarloIntegrator(props)
{
	...
	m_config.m_seedShift = Point2i(props.getInteger("seed_xoffset",0), props.getInteger("seed_yoffset",0));

	m_config.m_seed = props.getInteger("seed", 0);
	...
	//identification of current pass
	m_config.m_isBase = props.getBoolean("isBase", true);
	m_config.m_isTimeOffset = props.getBoolean("isTimeOffset", false);

	//adaptive sampling settings
	m_config.m_useAdaptive = props.getBoolean("useAdaptive", false);
	m_config.m_sampling_iter = props.getInteger("samplingIterations", 4);

	//checks for valid parameters
	if (m_config.m_isBase && m_config.m_isTimeOffset){
		Log(EError, "A pass cannot be the time-offset pass and the base-pass at the same time!");
	}

	if (!m_config.m_useAdaptive && m_config.m_sampling_iter > 1){
		Log(EWarn, "Uses uniform sampling with more than one sampling iteration! Setting samplingIteration to one...");
		m_config.m_sampling_iter = 1;
	}

	if (m_config.m_useAdaptive && m_config.m_sampling_iter == 1){
		Log(EWarn, "Uses adaptive sampling with only one sampling iteration! Setting useAdaptive to false...");
		m_config.m_useAdaptive = false;
	}

	if (m_config.m_shiftThreshold<0.f){
		Log(EWarn, "Shift threshold is negative! Setting threshold to zero...");
		m_config.m_shiftThreshold = 0.f;
	}
}
...
void GradientPathIntegrator::renderBlock(const Scene *scene, const Sensor *sensor, Sampler *sampler, MotionMap *motionMap, SamplingMap *samplingMap, GPTWorkResult* block,
	const bool &stop, const std::vector< TPoint2<uint8_t> > &points) const
{
	...
	Point2i mv_offset; //motionVector-guided offset
	...
	for (size_t i = 0; i < points.size(); ++i) {
		...
		std::vector<Spectrum> v_mean(5, Spectrum(0.f));
		std::vector<Spectrum> v_meanSqr(5, Spectrum(0.f));

		int currentSample = 0;

		// get pixel correspondence if useMotionVectors are used and current rendering pass is a time-offset	
		mv_offset = (m_config.m_useMotionVectors && m_config.m_isTimeOffset) ? motionMap->getBackwardOffset(int(offset.x), int(offset.y)) : Point2i(0);

		if (mv_offset.x == std::numeric_limits<int>::max() || mv_offset.y == std::numeric_limits<int>::max()){	//new mv
			continue;
			//mv_offset = Point2i(0);
		}

		Point2i offset_full = Point2i(offset.x + mv_offset.x, offset.y + mv_offset.y);

		int sampleCount = samplingMap->get(offset_full.x, offset_full.y);


		int sampleCountRight = samplingMap->get(offset.x + 1, offset.y);
		int sampleCountBottom = samplingMap->get(offset.x, offset.y + 1);
		int sampleCountLeft = samplingMap->get(offset.x - 1, offset.y);
		int sampleCountTop = samplingMap->get(offset.x, offset.y - 1);
		...
		Float alphas[4];
		alphas[RIGHT]	= Float(sampleCount) / Float(sampleCount + sampleCountRight);
		alphas[BOTTOM]	= Float(sampleCount) / Float(sampleCount + sampleCountBottom);
		alphas[LEFT]	= Float(sampleCount) / Float(sampleCount + sampleCountLeft);
		alphas[TOP]		= Float(sampleCount) / Float(sampleCount + sampleCountTop);


		Variance<Spectrum> var_features[gpt::FEATURE_COUNT] = {};
		Variance<Spectrum> var_primal;
		Variance<Spectrum> var_dx_right;
		Variance<Spectrum> var_dx_left;
		Variance<Spectrum> var_dy_top;
		Variance<Spectrum> var_dy_bottom;

		for (size_t j = 0; j < sampleCount/*sampler->getSampleCount()*/; ++j) {

			if (stop)
				break;

			//set sample offset if several sampling iterations are used, this is only the case if we use adaptive sampling and are currently rendering the base pass.
			int ac = (m_config.m_useAdaptive && m_config.m_isBase)? samplingMap->getAccum(offset.x, offset.y) : 0;
			// resetRNG only works with a deterministic sampler!
			sampler->resetRNG(offset_full + m_config.m_seedShift, m_config.m_seed, 0, j + ac); //new mv
			...
			//add offset sample in case useMotionVector is active
			Point2 mv_samplePos(samplePos); //new mv
			...
			Spectrum features[gpt::FEATURE_COUNT] = {
				Spectrum(0.0f), // FEAT_ALBEDO
				Spectrum(0.0f), // FEAT_NORMAL
				Spectrum(0.0f), // FEAT_POSITION
				Spectrum(0.0f), // FEAT_DEPTH
				Spectrum(0.0f), // FEAT_VISIBILITY
				Spectrum(0.0f), // FEAT_DIFFUSE
				Spectrum(0.0f)  // FEAT_SPECULAR
			};
			bool featuresWritten = false;

			tracer.evaluatePoint(rRec, mv_samplePos, apertureSample, timeSample, diffScaleFactor, centralVeryDirect, centralThroughput, gradients, shiftedThroughputs, alphas, features, &featuresWritten, NULL, !m_config.m_disableGradients);

			// Update feature variances.
			{
				// Feature variances.
				if (features != nullptr && featuresWritten) {
					for(int i = 0; i < gpt::FEATURE_COUNT; ++i) {
						var_features[i].addSample(features[i]);
					}
				}

				// Primal, dx and dy variances.
#ifndef CENTRAL_RADIANCE
#error "Only central radiance supported currently"
#endif
				var_primal.addSample(centralThroughput);
				if(!m_config.m_disableGradients) {
					var_dx_right.addSample(gradients[RIGHT]);
					var_dx_left.addSample(gradients[LEFT]);
					var_dy_top.addSample(gradients[TOP]);
					var_dy_bottom.addSample(gradients[BOTTOM]);
				}
			}
			...
			{
#ifdef CENTRAL_RADIANCE
				if(centralThroughput[0] < 0.0f || centralThroughput[1] < 0.0f || centralThroughput[2] < 0.0f) {
					Log(EError, "Negative color values!");
				}
			...
			{
#ifdef CENTRAL_RADIANCE
				//	if (m_config.m_currFrame>0 && m_config.m_useMotionVectors)
			...
			// Store the features.
			if (features != nullptr && featuresWritten) {
				block->put(samplePos, features[0], 1.0f, 1.0f, FEAT_ALBEDO);
				block->put(samplePos, features[1], 1.0f, 1.0f, FEAT_NORMAL);
				block->put(samplePos, features[2], 1.0f, 1.0f, FEAT_POSITION);
				block->put(samplePos, features[3], 1.0f, 1.0f, FEAT_DEPTH);
				block->put(samplePos, features[4], 1.0f, 1.0f, FEAT_VISIBILITY);
				block->put(samplePos, features[5], 1.0f, 1.0f, FEAT_DIFFUSE);
				block->put(samplePos, features[6], 1.0f, 1.0f, FEAT_SPECULAR);
			}

			// update variance using online algorithm (Knuth)
			Spectrum delta = centralThroughput - v_mean[0];
			v_mean[0] += delta / (currentSample + 1);
			v_meanSqr[0] += delta * (centralThroughput - v_mean[0]);
			...
			if (!m_config.m_disableGradients){
				// Gradients.
				{
					block->put(left_pixel, -gradients[LEFT], 1.0f, 1.0f, BUFFER_DX);    // Negative x gradient.
					block->put(center_pixel, gradients[RIGHT], 1.0f, 1.0f, BUFFER_DX);  // Positive x gradient.
					block->put(top_pixel, -gradients[TOP], 1.0f, 1.0f, BUFFER_DY);      // Negative y gradient.
					block->put(center_pixel, gradients[BOTTOM], 1.0f, 1.0f, BUFFER_DY); // Positive y gradient.
				}
				
				for (int i=0; i < 4; ++i){
					delta = gradients[i] - v_mean[i + 1];
					v_mean[i + 1] += delta / (currentSample + 1);
					v_meanSqr[i + 1] += delta * (gradients[i] - v_mean[i + 1]);
				}
			}
		...
		if (currentSample >= 2){
			// Variance for adaptive sampling and sampling map.
			Point2 centralPixel = Point2(offset.x + 0.5f, offset.y + 0.5f);
			float weight = currentSample*currentSample / (currentSample - 1); // n^2 * (m2 / (n-1))
			block->put(centralPixel, weight*v_meanSqr[0], 1.0f, 1.0f, SAMPLING_MAP_VARIANCE_TP);

			// Variance used as a feature. Adaptive sampling not supported!
			block->put(centralPixel, var_primal.getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_PRIMAL);
		}

		if (!m_config.m_disableGradients){
			//store variance
			//we sum up the variance of the primal image and dx and dy
			//hence we have alpha*var(I(x,y)) + var(0.5*dx_pos(x,y)+0.5*dx_neg(x+1,y)) + var(0.5*dy_pos(x,y)+0.5*dy_neg(x,y+1)) 
			// =  alpha*var(I(x,y)) + 0.25*var(dx_pos(x,y))+0.25*var(dx_neg(x+1,y)) + 0.25*var(dy_pos(x,y))+0.25*var(dy_neg(x,y+1)) 

			//Spectrum variance = Spectrum(0.f);
			//add 0.5 to pixel since we want to count it for the central pixel.

			Point2 centralPixel = Point2(offset.x + 0.5f, offset.y + 0.5f);
			Point2 leftPixel	= Point2(offset.x + 0.5f, offset.y + 0.5f) - Vector2(1.0f, 0.0f);
			Point2 topPixel		= Point2(offset.x + 0.5f, offset.y + 0.5f) - Vector2(0.0f, 1.0f);

			// Variances for adaptive sampling and sampling map.
			if (currentSample >= 2) {
				//weight is for mixing variance from passes. Since we add only 1 value per pass mitsuba will combine them as (pass1 + pass2)/2
				//assuming we have n1 samples in pass 1 and n2 samples in pass2 then to get the correct variance we have to weight the values with
				//(n1^2 * pass1 + n2^2 * pass2) / (n1+n2)^2
				//division by (n1+n2)^2 occurs later in sampling_map.cpp
				float weight_v = currentSample*currentSample / (currentSample - 1); //division by (currentSample - 1) is to get the sampleVariance (see Knuth algorithm) 
				float weight_s = sqrt(currentSample);

				// Variance
				block->put(leftPixel,		(weight_v*0.25f)*v_meanSqr[1 + LEFT],    1.0f, 1.0f, SAMPLING_MAP_VARIANCE_DX); // n^2 * (m2 / (n-1))
				block->put(centralPixel,	(weight_v*0.25f)*v_meanSqr[1 + RIGHT],   1.0f, 1.0f, SAMPLING_MAP_VARIANCE_DX);
				block->put(topPixel,		(weight_v*0.25f)*v_meanSqr[1 + TOP],     1.0f, 1.0f, SAMPLING_MAP_VARIANCE_DY);
				block->put(centralPixel,	(weight_v*0.25f)*v_meanSqr[1 + BOTTOM],  1.0f, 1.0f, SAMPLING_MAP_VARIANCE_DY);
			}

			// Variances for gradients (for the feature buffers).
			if (currentSample >= 2)
			{
				// Output needs to be
				//     var(0.5 * dx1 + 0.5 * dx2) = 0.25 var(dx1) + 0.25 var(dx2).
				//
				// Adding samples 0.5 var(dx1) and 0.5 var(dx2) will result in average 0.25 var(dx1) + 0.25 var(dx2).
				block->put(leftPixel,       0.5f * var_dx_left.getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_DX);
				block->put(centralPixel,    0.5f * var_dx_right.getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_DX);
				block->put(topPixel,        0.5f * var_dy_top.getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_DY);
				block->put(centralPixel,    0.5f * var_dy_bottom.getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_DY);
			}
		}

		// Variances of the actual features.
		if (currentSample >= 2) {
			Point2 centralPixel = Point2(offset.x + 0.5f, offset.y + 0.5f);

			block->put(centralPixel, var_features[0].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_ALBEDO);
			block->put(centralPixel, var_features[1].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_NORMAL);
			block->put(centralPixel, var_features[2].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_POSITION);
			block->put(centralPixel, var_features[3].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_DEPTH);
			block->put(centralPixel, var_features[4].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_VISIBILITY);
			block->put(centralPixel, var_features[5].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_DIFFUSE);
			block->put(centralPixel, var_features[6].getSampleVarianceOfMean(), 1.0f, 1.0f, FEAT_VAR_SPECULAR);
		}
	}
}
	...
	int sceneResID, int sensorResID, int samplerResID)
{
	...
	if(!m_config.m_reconstructL1 && !m_config.m_reconstructL2) {
		if(job->isInteractive()) {
			Log(EWarn, "Running GPT interactively with reconstruction disabled in config. Enabling L1 reconstruction!");
			m_config.m_reconstructL1 = true;
		} else {
	...
	/* Adapt shutter open duration to number of frames. */
	float shutterOpen = sensor->getShutterOpen();
	float shutterOpenTimePerFrame = sensor->getShutterOpenTime();
	...
	size_t sampleCount = (m_config.m_useAdaptive && m_config.m_isBase) ? int(sampler->getSampleCount()) / m_config.m_sampling_iter : sampler->getSampleCount();
	if ((m_config.m_useAdaptive && m_config.m_isBase) && sampleCount < 4) {
		Log(EError, "Can't generate sampling map with less than 4 samples per iteration!");
	}
	...
	ref<Bitmap> varianceBitmap(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
	ref<Bitmap> varianceDxBitmap(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
	ref<Bitmap> varianceDyBitmap(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));

	ref<Bitmap> motionBitmap(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
	ref<Bitmap> motionBitmapInv(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));

	/* container for motion information that can be accessed by the render blocks */
	ref<MotionMap> motion_map = new MotionMap(film->getCropSize().x, film->getCropSize().y);

	/* sampling map computation */
	ref<SamplingMap> sampling_map = new SamplingMap(sampleCount, sampleCount * 8, film->getCropSize().x, film->getCropSize().y, 123098765, !m_config.m_disableGradients);

	std::vector<std::string> outNames;
	std::vector<bool> outWrite;

	std::string frame_name = "";
	outNames = gpt::bufferNames();

	int motionOutIdx = 0;
	int sppIdx = outNames.size() - 3;
	int motionInvOutIdx = outNames.size() - 2;
	int recOutIdx = outNames.size() - 1;

	//specify which buffers to write to disk. bitmaps entries correspond to what was defined in outNames (true = write, false = don't write)
	//doesn't work with DEBUG_BUFFERS
	bool output_motion = (m_config.m_isBase && m_config.m_useMotionVectors);

	outWrite ={ output_motion, 1, 1, 1, 1,                                  // motion, primal, dx, dy, direct
				0, 0, 0,													// var_primal, var_dx, var_dy  
				1, 1, 1, 1, 1,          // albedo, normals, position, depth, visibility
				1, 1, 1, 1, 1,          // var for albedo, normals, position, depth, visibility
				1, 1, 1,                // var for primal, dx and dy (as features)
				1, 1, 1, 1,             // diffuse, specular, their variances
				(m_config.m_isBase && m_config.m_useAdaptive), (m_config.m_isBase && m_config.m_useMotionVectors), // sampling_map, motion_map_inv
				(m_config.m_reconstructL1 || m_config.m_reconstructL2)												//only write reconstruction if asked 
	};



	if (!film->setBuffers(outNames, outWrite)){
		Log(EError, "Cannot render image! G-PT has been called without MultiFilm.");
		return false;
	}


	/* Compute motion vector. writes directly into buffer with 0 */
	if (m_config.m_isBase && m_config.m_useMotionVectors){
		Float current_time = shutterOpen + 0.5f*shutterOpenTimePerFrame; // "halfway"-time during shutter of current frame
		float time_between_frames = 1.f; // TODO: we assume the time between two frames is 1, this might change and require a user parameter!
		motionBitmap = BidirectionalUtils::getMotionVectors(scene, sceneResID, sensorResID, queue, job, current_time, current_time + 1.f, 1);


		Log(EInfo, "motion-map was successfully generated...");
		motion_map->setForwardMap(motionBitmap->getFloatData());

		ref<Bitmap> motionBitmapReverse = BidirectionalUtils::getMotionVectors(scene, sceneResID, sensorResID, queue, job, current_time + 1.f, current_time, 1);				
		motion_map->makeConservative(motionBitmapReverse->getFloatData());

		Log(EInfo, "motion-map was successfully processed...");
	}

	ref<BlockedRenderProcess> proc = new GPTRenderProcess(job, queue, scene->getBlockSize(), m_config);

	//non base paths only need one iteration, since they can read the (accumulated) sampling map directly from a file!
	int n_iter = (m_config.m_useAdaptive && !m_config.m_isBase)? 1 : m_config.m_sampling_iter;// (m_config.m_useMotionVectors || !m_config.m_useAdaptive) ? 1 : 2;

	// Render all iterations of adaptive sampling (and also the case with no adaptive sampling).
	for (int sampling_iter=0; sampling_iter < n_iter; sampling_iter++){

		//read sampling map if adaptive sampling is used and we are currently *not* rendering the base pass
		if (m_config.m_useAdaptive && !m_config.m_isBase){
			fs::path outname = getMapPath(scene, int(shutterOpen), m_config.m_seed, "-spp");
			ref<FileStream> smFile   = new FileStream(outname, FileStream::EReadOnly);
			ref<Bitmap> smBitmap = new Bitmap(Bitmap::EOpenEXR, smFile);
			sampling_map->setSamplingMap(smBitmap->getFloat16Data()); //bitmap is 16bit
			Log(EInfo, "sampling map found and loaded!");
		}

		//read motion map if motion vectors are used and we are currently rendering a time offset pass
		if (m_config.m_useMotionVectors && m_config.m_isTimeOffset){
			fs::path outname = getMapPath(scene, int(shutterOpen), m_config.m_seed, "-motion");
			ref<FileStream> mFile   = new FileStream(outname, FileStream::EReadOnly);
			motionBitmap= new Bitmap(Bitmap::EOpenEXR, mFile);
			motion_map->setForwardMap(motionBitmap->getFloat16Data()); //aBitmap is 16bit
			Log(EInfo, "motion map found and loaded!");

			//get inv motion map
			outname = getMapPath(scene, int(shutterOpen), m_config.m_seed, "-motion-inv");
			mFile   = new FileStream(outname, FileStream::EReadOnly);
			motionBitmap= new Bitmap(Bitmap::EOpenEXR, mFile);
			motion_map->setBackwardMap(motionBitmap->getFloat16Data()); //aBitmap is 16bit
			Log(EInfo, "backward motion map found and loaded!");
		}
		...
		int motionMapResID = sched->registerResource(motion_map);
		int samplingMapResID = sched->registerResource(sampling_map);
		proc->bindResource("integrator", integratorResID);
		proc->bindResource("motionMap", motionMapResID);
		proc->bindResource("samplingMap", samplingMapResID);
		...
		if (!proc->getReturnStatus())
			return false;

		//update accumulation map *after* renderpass
		sampling_map->updateAccum();

		ref<Bitmap> sppBitmap2(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));

		sampling_map->getImageAccum(sppBitmap2);

		film->setBitmapMulti(sppBitmap2, 1, sppIdx);

		//project each pixel of offset image onto pixel correspondence of last frame
		if (m_config.m_useMotionVectors && m_config.m_isTimeOffset){
			//get un-projected data
			...
			//alloc new buffers
			ref<Bitmap> throughputBackProjected(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
			ref<Bitmap> dxBackProjected(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
			ref<Bitmap> dyBackProjected(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
			//backprojectthroughputBitmap
			motion_map->backProjectBuffer(throughputBitmap->getFloatData(), throughputBackProjected->getFloatData());
			motion_map->backProjectBuffer(dxBitmap->getFloatData(), dxBackProjected->getFloatData());
			motion_map->backProjectBuffer(dyBitmap->getFloatData(), dyBackProjected->getFloatData());
			//put new data 
			film->setBitmapMulti(throughputBackProjected, 1, BUFFER_THROUGHPUT);
			film->setBitmapMulti(dxBackProjected, 1, BUFFER_DX);
			film->setBitmapMulti(dyBackProjected, 1, BUFFER_DY);
		}

		//compute sampling map for next sampling iteration if adaptive sampling is used and we are currently rendering the base pass
		//for the final pass this is not required.
		if (m_config.m_useAdaptive && m_config.m_isBase && sampling_iter < n_iter - 1)
		{
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), throughputBitmap, BUFFER_THROUGHPUT);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), dxBitmap, BUFFER_DX);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), dyBitmap, BUFFER_DY);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), varianceBitmap, SAMPLING_MAP_VARIANCE_TP);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), varianceDxBitmap, SAMPLING_MAP_VARIANCE_DX);
			film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), varianceDyBitmap, SAMPLING_MAP_VARIANCE_DY);
			if (sampleCount >= 2)
				sampling_map->computeMapWithGradients(varianceBitmap->getFloatData(), varianceDxBitmap->getFloatData(), varianceDyBitmap->getFloatData(), 
													  throughputBitmap->getFloatData(), dxBitmap->getFloatData(), dyBitmap->getFloatData(), sampleCount);
		}
	}

	//if motion vectors are used write them to disk
	if (m_config.m_useMotionVectors){
		setBitmapFromArray(motionBitmap, motion_map->getForwardMap());
		film->setBitmapMulti(motionBitmap, 1, motionOutIdx);

		setBitmapFromArray(motionBitmapInv, motion_map->getBackwardMap());
		film->setBitmapMulti(motionBitmapInv, 1, motionInvOutIdx);
	}

	// Reconstruct.

	if(m_config.m_reconstructL1 || m_config.m_reconstructL2) {
		SLog(EInfo, "Reconstructing from gradients.");
		...
		solver.importImagesMTS(dxVector.data(), dyVector.data(), nullptr, throughputVector.data(), film->getCropSize().x, film->getCropSize().y);

		solver.setupBackend();
		
		/* Reconstruct! */
		solver.solveIndirect(); // (Note: Solves direct too.)
		...
		if (job->isInteractive()){
			film->setBitmapMulti(reconstructionBitmap, 1, BUFFER_FINAL); //screen buffer
		}
		film->setBitmapMulti(reconstructionBitmap, 1, recOutIdx); //output buffer for writing to disc
#endif
	}
...
std::string GradientPathIntegrator::toString() const {
	...
	oss << "GradientPathTracer[" << endl
		...
		<< "  seed = " << m_config.m_seed << "," << endl
		<< "  shiftThreshold = " << float(m_config.m_shiftThreshold) << "," << endl
		...
		<< "  useMotionVectors = " << (m_config.m_useMotionVectors ? "True" : "False") << "," << endl
		<< "  useAdaptive = " << (m_config.m_useAdaptive ? "True" : "False") << "," << endl
		<< "  samplingIter = " << (m_config.m_sampling_iter) << "," << endl
		<< "  isBase = " << (m_config.m_isBase ? "True" : "False") << "," << endl
		<< "  isTimeOffset = " << (m_config.m_isTimeOffset ? "True" : "False") << "," << endl
		<< "]";
	return oss.str();
}
