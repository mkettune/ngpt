...
public:
	...
	void prepare() {
		...
		m_motionMap = static_cast<MotionMap *>(getResource("motionMap"));
		m_samplingMap = static_cast<SamplingMap *>(getResource("samplingMap"));
		...
		m_boxFilter = static_cast<ReconstructionFilter *> (PluginManager::getInstance()->
				createObject(MTS_CLASS(ReconstructionFilter), Properties("box")));
		m_boxFilter->configure();
	}
	...
	void process(const WorkUnit *workUnit, WorkResult *workResult,
		...
		m_integrator->renderBlock(m_scene, m_sensor, m_sampler, m_motionMap, m_samplingMap, 
...
private:
	...
	ref<MotionMap> m_motionMap;
	ref<SamplingMap> m_samplingMap;
	...
	ReconstructionFilter *m_boxFilter; ///< Box filter for features.
};
...
void GPTRenderProcess::processResult(const WorkResult *result, bool cancelled) {
	...
	for(size_t i = 0; i < gpt::BUFFER_COUNT; ++i) {
		m_film->putMulti(block->getImageBlock(i), i);
		//m_film->addBitmapMulti(m_result->getImageBlock(i)->getBitmap(), Float(1.0), i);
	}
