...
void SamplingIntegrator::renderBlock(const Scene *scene,
	...
	for (size_t i = 0; i<points.size(); ++i) {
		...
		for (size_t j = 0; j<sampler->getSampleCount(); j++) {
			// Set the seed for the deterministic sampler. Does nothing for other samplers.
			sampler->resetRNG(offset + m_seedShift, m_seed, 0, j);
...
MonteCarloIntegrator::MonteCarloIntegrator(const Properties &props) : SamplingIntegrator(props) {
	...
	m_seed = (uint64_t)props.getInteger("seed", 0);

	m_seedShift = Point2i(props.getInteger("seed_xoffset",0), props.getInteger("seed_yoffset",0));
