...
public:
	...
	MultiFilm(const Properties &props) : Film(props) {
		...
		std::string fileFormat = boost::to_lower_copy(
			props.getString("fileFormat", "pfm"/*"openexr"*/));
	...
	/*	m_numBuffers = 1;
		...
		*/
		...
		std::vector<bool> defaultOutWrite ={1};
		setBuffers(defaultNames, defaultOutWrite);
	}
	...
	bool setBuffers(std::vector<std::string> &names, std::vector<bool> &outWrite){

		Assert(names.size() == outWrite.size());

		m_numBuffers = names.size();
		m_storage.resize(m_numBuffers);
		m_ext_name = names;
		m_ext_write = outWrite;
	...
	bool renameBuffers(std::vector<std::string> &names, std::vector<bool> &outWrite){

		Assert(names.size() == outWrite.size());

		if (names.size() != m_numBuffers)
			return false;
		m_ext_name = names;
		m_ext_write = outWrite;
		return true;
...
protected:
	...
	std::vector<bool> m_ext_write; //names of the buffers
	int m_numBuffers;
};
