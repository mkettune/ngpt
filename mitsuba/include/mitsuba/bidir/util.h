...
public:
	/**
	* \brief Call motion integrator to obtain the current frame's motion vectors
	*/
	static ref<Bitmap> getMotionVectors(Scene *scene, int sceneResID,
		int sensorResID, RenderQueue *queue, const RenderJob *job, float current_time, float target_time, size_t num_samples);
	/**
